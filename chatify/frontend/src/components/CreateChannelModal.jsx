      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700/50">
          <h3 className="text-slate-100 font-semibold text-base flex items-center gap-2">
            ...
            {createdChannel ? t("createChannel.createdTitle") : t("createChannel.title")}
          </h3>
        ...
        {createdChannel ? (
          <div className="p-5 space-y-4">
            <p className="text-slate-300 text-sm text-center">
              {t("createChannel.createdSuccess", { name: createdChannel.name })}
            </p>

            {createdChannel.is_public && (
              <div className="bg-slate-900/40 rounded-lg p-3">
                <p className="text-slate-400 text-xs mb-1.5">{t("createChannel.inviteCodeLabel")}</p>
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-4">
            <div>
              <label className="text-slate-400 text-xs mb-1 block">{t("createChannel.nameLabel")}</label>
              <input
                autoFocus
                type="text"
                value={name}
            ...
            <div>
              <label className="text-slate-400 text-xs mb-1 block">{t("createChannel.descLabel")}</label>
              <textarea
                value={description}
            ...
            <div className="flex items-center justify-between bg-slate-900/40 rounded-lg px-3 py-2.5">
              <div>
                <p className="text-slate-300 text-sm">{t("createChannel.publicLabel")}</p>
                <p className="text-slate-500 text-xs">{t("createChannel.publicSub")}</p>
              </div>
              <button
                type="button"
            ...
            >
              {isSubmitting ? <LoaderIcon className="w-4 h-4 animate-spin" /> : t("createChannel.submit")}
            </button>
          </form>
        )}
      </div>
