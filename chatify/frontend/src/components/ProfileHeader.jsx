...
function ProfileHeader({ onNewGroup }) {
  ...
  const handleNewItemSubmit = async () => {
    ...
    if (newItemType === "group") {
      try {
        const res = await axios.post(
          "http://localhost:8000/groups/groups/",
          {
            name: newItemName,
            description: "گروه جدید" // اضافه کردن description پیش‌فرض
          },
          {
            headers: {
        ...
        if (onNewGroup) onNewGroup(res.data); // اضافه کردن گروه به لیست
        setNewItemName("");
        setNewItemType(null);
    ...
    // مخاطب هم می‌تواند اضافه شود اینجا مشابه گروه
  };
          ...
          <div className="relative flex items-center gap-2">
            ...
            {showNewMenu && !newItemType && (
              <div
                ...
                <div className="p-2 hover:bg-slate-700 cursor-pointer text-white"
                     onClick={() => setNewItemType("contact")}>
                  ساخت مخاطب
                </div>
              </div>
            )}
            ...
            {newItemType && (
              <div
                ...
                <p className="text-white mb-1">نام {newItemType === "group" ? "گروه" : "مخاطب"} را وارد کنید:</p>
                <input
                  type="text"
                  value={newItemName}
                  onChange={(e) => setNewItemName(e.target.value)}
                  className="w-full p-1 rounded bg-slate-600 text-white outline-none"
                  onKeyDown={(e) => { if (e.key === "Enter") handleNewItemSubmit(); }}
                />
                <button
