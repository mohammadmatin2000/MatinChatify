...
function LoginPage() {
    ...
    const {login, requestOtp, verifyOtp, verify2FA} = useAuthStore();

    const [twoStepUserId, setTwoStepUserId] = useState(null);
    ...
    const handleEmailSubmit = async (e) => {
        ...
        if (result?.twoStepRequired) {
            setTwoStepUserId(result.userId);
            return;
        }
        if (result) navigate("/");
    ...
    const handleVerify2FA = async (e) => {
        ...
        const user = await verify2FA(twoStepUserId, twoStepCode);
                    ...
                    <div className="w-full flex flex-col md:flex-row">
                        ...
                        <div className="md:w-1/2 p-8 flex items-center justify-center md:border-l border-slate-600/30">
                            <div className="w-full max-w-md">
                                ...
                                {/* فرم ایمیل معمولی — فقط وقتی هنوز مرحله‌ی دومرحله‌ای شروع نشده */}
                                {tab === "email" && !twoStepUserId && (
                                ...
                                {/* فرم کد تایید دو مرحله‌ای — فقط وقتی login تشخیص داد لازمه */}
                                {tab === "email" && twoStepUserId && (
                                    <form onSubmit={handleVerify2FA} className="space-y-6">
                                        <p className="text-slate-400 text-sm text-center">
                                            کد تایید به ایمیلت ارسال شد. وارد کن:
                                        </p>
                                        ...
                                        <div>
                                            ...
                                            <div className="relative">
                                                ...
                                                <input
                                                    ...
                                                    value={twoStepCode}
                                                    onChange={(e) =>
                                                        setTwoStepCode(e.target.value.replace(/\D/g, ""))
                                                    }
                                        ...
                                        <button
                                            className="auth-btn w-full flex items-center justify-center"
                                            type="submit"
                                            disabled={isVerifying2FA || twoStepCode.length !== 6}
                                        >
                                            {isVerifying2FA ? (
                                                <LoaderIcon className="w-5 h-5 animate-spin"/>
                                            ) : (
