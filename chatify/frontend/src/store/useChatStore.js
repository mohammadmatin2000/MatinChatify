  ...
  getMyChatPartners: async () => {
    ...
    try {
      ...
      const formattedChats = data.map((chat) => ({
        ...
        fullName: chat.name || chat.contact_name || `Contact ${chat.id || chat._id}`,
        profilePic: chat.profilePic || chat.avatar || "/avatar.png",
        lastMessage: chat.lastMessage || "",
        lastMessageAt: chat.lastMessageAt ? safeDate(chat.lastMessageAt) : new Date(0),
        raw: chat,
      }));

      formattedChats.sort((a, b) => b.lastMessageAt - a.lastMessageAt);
